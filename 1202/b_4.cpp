...
int main() {
	...
	for (int i=0; i<=N; i++) {
		for (int j=0; j<=2; j++) {
			if (j==0) {
				twoDimVector[i][j] = (i / 6) + (i % 6 != 0);
			} else if (j==1) {
				// for loopで、
				for (int k=0; k<i; k++) {
					twoDimVector[i][j] = min(twoDimVector[i][j], twoDimVector[k][j-1] + (i-k)/8 + ((i-k)%8 != 0));
				}
				...
				for (int k=0; k<i; k++) {
					...
					twoDimVector[i][j] = min(twoDimVector[i][j], twoDimVector[k][j-1] + (i-k)/12 + ((i-k)%12 != 0));
				}
				twoDimVector[i][j] = min(twoDimVector[i][j], twoDimVector[i][j-1]);
			}
		}
